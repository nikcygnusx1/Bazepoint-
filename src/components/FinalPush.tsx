    ...
    >
      ...
      <div className="max-w-3xl mx-auto px-6 text-center relative z-10">
        <motion.div style={{ y: fpHeadlineY, opacity: fpHeadlineOpacity }} className="will-change-transform">
          {/* OUTCOME HEADLINE — echoes thesis one final time */}
          <h2 id="final-title" className="text-4xl md:text-5xl lg:text-6xl font-display font-[800] text-[#1A1A18] mb-6 leading-tight tracking-[-1px]">
            Your brief. Verified factories.<br />First email, written.
          </h2>
          {/* SUBTEXT — removes friction, echoes "no experience needed" */}
          <p className="text-base md:text-lg font-body text-[var(--color-bz-text-muted)] mt-4 mb-8 max-w-[420px] mx-auto leading-relaxed">
            Describe what you want to make. Baze does the rest — no sourcing experience, no broker fees, no weeks of waiting.
          </p>
        </motion.div>
        ...
        <motion.div style={{ y: fpCtaY, opacity: fpCtaOpacity }} className="flex flex-col sm:flex-row items-center justify-center gap-4 will-change-transform">
          ...
          >
            Describe your product — it's free
            <ArrowRight className="w-4 h-4 inline-block ml-2 transition-transform duration-150 ease-out group-hover:translate-x-1" />
          </motion.a>
          <span className="text-sm text-[var(--color-bz-text-faint)] font-body">Free to search. No credit card. No broker.</span>
        </motion.div>
      </div>
    </motion.section>
  );
