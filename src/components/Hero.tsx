...
export function Hero() {
  ...
  // THESIS: Bazepoint is an AI sourcing agent that takes your product brief,
  // finds verified factories, and writes your first outreach email.
  const headlineWords = ["Brief in.", "Verified", "factory", "out."];
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 z-[1] pointer-events-none" aria-hidden="true">
        <HeroCanvas isFocused={isInputFocused} isSearching={isSearching} className="w-full h-full" />
      </div>

      <div className="hero-ambient-bg z-0 relative" aria-hidden="true" />
      <div className="absolute inset-0 pointer-events-none opacity-50 z-0" style={{ backgroundImage: 'linear-gradient(135deg, transparent 0%, rgba(201,107,42,0.03) 100%)' }}></div>
      ...
      <div className="max-w-[1440px] mx-auto px-6 md:px-16 w-full relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-8 items-center">
        ...
        <div className="col-span-1 lg:col-span-7 flex flex-col justify-center items-center lg:items-start text-center lg:text-left">
          ...
          <h1 id="hero-title" className="text-[clamp(2rem,6vw,4rem)] leading-[1.05] font-display font-[800] text-[#B8E2F2] tracking-[-1px] md:tracking-[-3px] mb-6">
            <div className="flex flex-wrap justify-center lg:justify-start">
              {headlineWords.map((word, i) => (
                <motion.span key={i} className="inline-block mr-[0.25em]" variants={heroHeadline}>
                  {word}
                </motion.span>
              ))}
            </div>
            <div className="flex flex-wrap mt-2 justify-center lg:justify-start">
              <motion.span className="inline-block mr-[0.25em]" variants={heroHeadline}>First email</motion.span>
              <motion.span className="inline-block mr-[0.25em]" variants={heroHeadline}>written.</motion.span>
              <motion.span className="inline-block mr-[0.25em] opacity-50 line-through" variants={heroHeadline}>Weeks wasted.</motion.span>
            </div>
          </h1>

          {/* SINGLE-THESIS SUBTEXT */}
          <motion.p
          ...
          >
            Tell Baze what you want to make. It searches a vetted network of manufacturers across MENA, Southeast Asia, and Oceania, filters by your budget and MOQ, and writes your first outreach email — ready to send in under a minute.
          </motion.p>
          ...
          >
            ...
            <motion.div variants={heroStats} className="flex items-center gap-2 text-left">
              ...
              <span className="text-xs text-[var(--color-bz-text-faint)] font-body">
                Verified manufacturers only — no brokers, no Alibaba noise
              </span>
            </motion.div>
          </motion.div>

          {/* PROMPT CHIPS — outcome-oriented, not just product categories */}
          <motion.div
          ...
          >
            {[
              ...
              <motion.span key={i} variants={fadeUpFast}>
                <motion.button
